...
class ArchiveVC: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool){
        // load dummy data
        emailDict = dataAPIObj.fetchData(key: "archKey")
        tbl.reloadData()
    }
    
    @IBAction func deleteBtn(_ sender: UIButton) {
        dataAPIObj.DeleteEmail(key:"archKey", name:emailDict[sender.tag]["name"]!, subject:emailDict[sender.tag]["subject"]!)
        emailDict = dataAPIObj.fetchData(key: "archKey")
        tbl.reloadData()
    }
}
extension ArchiveVC:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell:TableViewCell = tableView.dequeueReusableCell(withIdentifier: "cell2")! as! TableViewCell
